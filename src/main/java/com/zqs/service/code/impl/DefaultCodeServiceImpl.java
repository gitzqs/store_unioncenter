...
import com.zqs.utils.api.APIConfig;
...
public class DefaultCodeServiceImpl implements ICodeService {
	...
	public String generate(MessageCode mc) {
		...
		if(!StringUtils.isEmpty(mobile) && type != 0){
			try{
				//3、发送验证码
				String validateCode = StringUtils.randomSix();  //6位随机数
				MessageCode code = new MessageCode();
				code.setMobile(mobile);
				code.setCode(validateCode);
				ReturnObject ro = WebClient.callRest(APIConfig.TOOL_SERVER + "code/generate", code);
				if(ro != null && ReturnCode.SUCCESS_CODE.equals(ro.getReturnCode())){
					//4、生成记录
